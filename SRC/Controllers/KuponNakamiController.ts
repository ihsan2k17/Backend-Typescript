...
import { SaveKupon_NakamiVM } from "../Database/Models/MKupon_Nakami";

export interface IKuponNakamiController {
...
@injectable()
class KuponNakamiController implements IKuponNakamiController {
    private _skupon: IKuponNakamiService;
    constructor(@inject('IKuponNakamiService') skupon: IKuponNakamiService) {
        this._skupon = skupon;
    }
    ...
    async addKupon(req: Request, res: Response): Promise<Response> {
        const {
            ...
            Hadiah, Nama_Hadiah, Periode, Jenis } = req.body as SaveKupon_NakamiVM;
    ...
    async updateDuration(req: Request, res: Response): Promise<Response> {
        ...
        const Input_Duration = req.body as SaveKupon_NakamiVM
        const User = req.app.locals.credential.Username;
        const currentDate = new Date();
        const formattedDate = `${currentDate.getDate()}/${currentDate.getMonth() + 1}/${currentDate.getFullYear()}`;
        try {
            const upduration = await this._skupon.updateDuration(
                Number(ID),
                Number(Poin),
                User,
                Number(Hadiah),
    ...
    async updateKupon(req: Request, res: Response): Promise<Response> {
        ...
        const { Kupon, Hadiah, Nama_Hadiah, Hadiah_ke, Tahun, Periode } = req.body as SaveKupon_NakamiVM ;
        const User = req.app.locals.credential.Username;
        const currentDate = new Date();
        const formattedDate = `${currentDate.getDate()}/${currentDate.getMonth() + 1}/${currentDate.getFullYear()}`;
        try {
    ...
    async updateVoucher(req: Request, res: Response): Promise<Response> {
        ...
        const { Voucher, Hadiah, Nama_Hadiah, Hadiah_ke, Tahun, Periode } = req.body as SaveKupon_NakamiVM;
        const User = req.app.locals.credential.Username;
        const currentDate = new Date();
        const formattedDate = `${currentDate.getDate()}/${currentDate.getMonth() + 1}/${currentDate.getFullYear()}`;
        try {
...
export default KuponNakamiController;