...
class KuponService implements IKuponService {
    ...
    async cekKuponbyIddanPoin(ID: number, Poin: number): Promise<KuponVM[]> {
        if (!ID) {
            throw new Error(`${ID} tidak ada`);
        }
        const cekkupon = await this._kuponRepository.cekKuponbyIddanPoin(ID, Poin);
        const response = cekkupon.map((kpn: Kupon) => {
            const vm = new KuponVM(kpn);
            return vm;
        });
    ...
    async createKupon(entity: SaveKuponVM): Promise<boolean> {
        try {
            ...
            if (typeof newdata.Poin !== 'number' || typeof newdata.Kupon !== 'number' || typeof newdata.Periode !=='number') {
    ...
    async createVoucher(entity: SaveKuponVM): Promise<Boolean> {
        try {
            ...
            // Pastikan Poin dan Kupon adalah angka
            if (typeof newdata.Poin !== 'number' || typeof newdata.Voucher !== 'number'|| typeof newdata.Periode !== 'number') {
                throw new Error('Poin dan Kupon harus berupa angka.');
            }
